...
answerBlock: |
  When every approval, exception, and status update has to pass
  through one manager, that person becomes both the bottleneck and
  the single point of failure: if they're on holiday, sick, or leave
  the business, the whole process stalls. AI automation doesn't
  replace that manager; it takes over the repeatable parts of their
  job so decisions keep moving without them personally touching every
  one of them.
