...
export function Header() {
  ...
  return (
    <>
      ...
      {/* ─── RIGHT: UTILITY CIRCLES (desktop) — Email + WhatsApp + Lang ─── */}
      <div
      ...
      >
        ...
        <a
          ...
          aria-label="Email"
          className="grid place-items-center rounded-full overflow-hidden group relative"
          style={{
            width: 40,
            height: 40,
            border: "1px solid rgba(255,255,255,.1)",
          ...
          onMouseMove={(e) => {
            if (contactRef.current) mag(e, contactRef.current, 0.45);
            cursor.set("link", t("cursor.email"));
          }}
          onMouseLeave={() => {
            if (contactRef.current) unmag(contactRef.current);
            cursor.reset();
            if (contactRef.current) {
              contactRef.current.style.borderColor = "rgba(255,255,255,.1)";
              contactRef.current.style.boxShadow = "none";
            }
          }}
          onMouseEnter={() => {
            if (contactRef.current) {
              contactRef.current.style.borderColor = "rgba(255,255,255,.3)";
              contactRef.current.style.boxShadow =
        ...
        >
          ...
          <Mail
            size={15}
            strokeWidth={1.5}
            className="relative z-10 text-white/50 group-hover:text-white transition-colors duration-300"
          />
        </a>
        ...
        <a
          ...
          onMouseMove={(e) => {
            if (waRef.current) mag(e, waRef.current, 0.45);
            cursor.set("link", t("cursor.whatsapp"));
          }}
          onMouseLeave={() => {
            if (waRef.current) unmag(waRef.current);
            cursor.reset();
            if (waRef.current) {
              waRef.current.style.borderColor = "rgba(255,255,255,.1)";
              waRef.current.style.boxShadow = "none";
            }
          }}
          onMouseEnter={() => {
            if (waRef.current) {
              waRef.current.style.borderColor = "rgba(255,255,255,.3)";
              waRef.current.style.boxShadow =
                "0 0 20px rgba(255,255,255,.08)";
            }
          }}
        >
